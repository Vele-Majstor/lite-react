...
function DataTable({
  ...
  const hasContextMenu = rows.some((row) => row._contextMenu);

  return (
    <Table
      zebraStyles={zebraStyles}
      overflowMenuOnHover={hasContextMenu}
      size={size}
    ...
    >
      <TableHead>
        <TableRow>
          ...
          {hasContextMenu && (
            <TableHeaders
              header={{ key: "overflowMenu", header: <span /> }}
            ></TableHeaders>
          )}
        </TableRow>
      </TableHead>
      <TableBody>
        {tableRows.map((row) => (
          <TableRow key={row._id}>
            ...
            {hasContextMenu && (
              <TableCell className="table-column-menu">
                {row._contextMenu && (
                  <ContextMenu
                    className="overflow-menu"
                    severity="ghost"
                    icon={<Ellipsis className="overflow-menu__icon" />}
                    iconTooltip={"Options"}
                    options={row._contextMenu}
                  />
                )}
              </TableCell>
            )}
