...
function MultiSelect<T>({
    ...
    function generateSelectedOptionLabels() {
        if (value && value.length > 0 && value.length < maxSelectedLabels) {
            return (
                <span className={styles.dropdown_value}>
                    {value.map((val, idx) => (
                        <button
                            key={idx}
                            className={styles.dropdown_value__chip}
                            onClick={(event) => {
                                event.preventDefault();
                                event.stopPropagation();
            ...
            return <span className={styles.dropdown_value}>{placeholder}</span>;
        }
    }
    ...
    function multiSelectItemTemplate(option: LabelValue<T>) {
        return (
            <div className={styles.dropdown_option}>
                <div className={styles.dropdown_option__container}>
                    <input
                        tabIndex={-1}
                        readOnly
                        type="checkbox"
                    ...
                    />
                    <span className={styles.dropdown_option__checkmark}></span>
                </div>
